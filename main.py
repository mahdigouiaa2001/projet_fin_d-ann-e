...
def index():
    if request.method == 'POST':
        ...
        with open("merged_info.txt", "r") as file:
            detected_text = ", ".join([line.strip().split()[-1] for line in file.readlines()])
        
        # Renvoyer le rendu HTML avec les images et le texte détecté
        return render_template('index.html', original_image='temp.jpg', processed_image='results.jpg', cropped_images=os.listdir(cropped_images_dir), detected_text=detected_text, log_demo=run_demo_output.stdout.decode())
...
def run_demo(transformation, feature_extraction, sequence_modeling, prediction, image_folder, saved_model):
    ...
    # Écrire le contenu de merged_info.txt dans le modèle HTML
    with open("merged_info.txt", "r") as merged_file:
        merged_text = merged_file.read()
